...
This repo uses the **local-markdown** issue tracker, so a "label" is the value of the `Status:` line near the top of the issue file under `.scratch/` (see `issue-tracker.md`). Apply one by editing that line — e.g. `Status: ready-for-agent`.
